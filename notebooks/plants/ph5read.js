// ph5read.js code for PhidgetFour_read.ipynb
...
// It includes a text box showing the status, and 5 text boxes to display values from the Phidget sensors. 
...
// the anywidget model includes five Python variables "temperature, humidity, moisture, luminance, pump" that will be
// kept updated between the JS front end and the Python kernel.
// A sixth variable "hubPort" allows the user to specify which port the DC power / pump is connected to. 
...
export function render({ model, el }) {
    ...
    async function openUSB() {
        if (conn) {
            ...
            try {await chPump1.close();} catch {console.log('chP.close failed');}
    ...
    function readDevices() {
        ...
        if (chPump1.attached) {p0 = chPump1.state; }       
